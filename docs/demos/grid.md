## Grid

### 演示

<img width="100" src="http://qr.topscan.com/api.php?text=http://aitter.oschina.io/#/grid"/>

<a href="http://aitter.oschina.io/#/grid" target="_blank" style="font-size:12px;color:#888;">demo 原始链接：http://aitter.oschina.io/#/grid</a>

<div style="width:377px;height:667px;display:inline-block;border:1px dashed #ececec;border-radius:5px;overflow:hidden;">
  <iframe src="http://aitter.oschina.io/#/grid" width="375" height="667" border="0" frameborder="0"></iframe>
</div>


### 使用示例

``` javascript
import React from 'react'
import { Grid, GridItem, GroupTitle } from 'mt-weui-react'
import Page from '../../component/page'

const icon = <img src="data:image/png;base64," />

var Demo = React.createClass({
  render() {
      return (
        <Page title="Grid">
          <GroupTitle>基础网格</GroupTitle>
          <Grid>
            <GridItem label="体检预约" icon={icon}></GridItem>
            <GridItem label="体检报告" icon={icon}></GridItem>
            <GridItem label="登录注册" icon={icon}></GridItem>
          </Grid>

          <GroupTitle>自定义内容</GroupTitle>
          <Grid>
            <GridItem>自定义内容1</GridItem>
            <GridItem>自定义内容2</GridItem>
            <GridItem>自定义内容3</GridItem>
          </Grid>
        </Page>
      );
  },
});

export default Demo

```
